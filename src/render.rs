...
impl Renderer for HtmlRenderer {
    fn render(
        ...
        html.push_str(format!("<html lang=\"{}\">\n", &renderer_config.main_lang).as_str());
        html.push_str("<head>\n");
        html.push_str("<meta charset=\"utf-8\">\n");
        html.push_str(&format!("<title>{}</title>\n", escape_html(&renderer_config.main_lang))); // TODO: should from category config
...
fn render_node(
    ...
    match node {
        ...
        Anchor::InnerUrlOpen { title, url } => {
            format!("  <a {}{}></a>", escape_html(url), escape_html(title))
        }
        Anchor::InnerUrlClose { title, url } => {
            format!("  <b {}{}></b>", escape_html(url), escape_html(title))
        }
        Anchor::InnerLoreOpen { title, path } => {
            ...
            format!("  <c {}{}></c>", escape_html(&href), escape_html(title))
        }
        Anchor::InnerLoreClose { title, path } => {
            ...
            format!("  <d {}{}></d>", escape_html(&href), escape_html(title))
        }
    }
}
